 ...
 */
//第三步：创建  算法类 进行顶点计算
public class Sjx2 extends OpenGLUtils {
    ...
    private void init() {
        ...
        for (int i = 0; i < 360 + length; i += length) {
            ...
            ver[count++] = (float) (0.5*(Math.cos(Math.toRadians(i)) - Math.sin(Math.toRadians(i))));
            ver[count++] = (float) (0.5*(Math.cos(Math.toRadians(i)) + Math.sin(Math.toRadians(i))));
            ver[count++] = 0;
        }
    ...
    public void drawSelf(GL10 gl) {
        ...
        gl.glDrawArrays(GL10.GL_TRIANGLE_STRIP, //绘制的图元
